...
app.post('/insert/:database/:collection', async (req, res) => {
    try {
        ...
        res.status(201).json({
            message : `Document saved success`, document: newDocument
        })
    } catch (err) {
...
app.put('/update/:database/:collection/:id', async (req, res) => {
    try {
        ...
        if (!updatedDocument) { 
            return res.status(404).json({message : ` An error occured, document hasn't got found`})
        }
        // Log a success message to the console
